...
returns jsonb language sql volatile security definer set search_path=public,pg_temp as $$
  select jsonb_build_object('scope','discovery','daily_limit',20,'daily_used',count(*),
  ...
  where kind='discovery' and created_at >= (date_trunc('day',now() at time zone 'America/Santiago') at time zone 'America/Santiago');
$$;
...
begin
  ...
  v_reason:=substring(p_error from '^ResearchDeferred: (INSUFFICIENT_BALANCE|RATE_LIMIT)$');
  -- A worker's free-text error alone cannot pause a run.
  if v_reason in ('INSUFFICIENT_BALANCE','RATE_LIMIT') then
  ...
  if v_defer then
    v_pause:=jsonb_build_object('reason_code',v_reason,'auto_resume',false);
...
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare v_run public.prospecting_runs%rowtype;
begin
  perform public.prospecting_require_roles(array['administrador']);
  perform pg_advisory_xact_lock(83276092);
...
begin
  perform pg_advisory_xact_lock(83276092);
  -- Release only explicitly scheduled quota pauses, without waiting for midnight.
  -- Manual, balance, cancelled and historical unclassified pauses stay untouched.
  ...
  loop
      ...
      insert into public.prospecting_events(run_id,level,stage,message)
        values(v_run_id,'info','enrichment_resumed','Investigacion reanudada: analisis de candidatos sin tope diario interno');
  end loop;
