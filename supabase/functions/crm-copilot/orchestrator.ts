...
export async function runOrchestrator(options: OrchestratorOptions) {
  ...
  const instructions = [
    ...
    "Para inventario total, cantidad total, stock valorizado, costo total o valor potencial de venta del agente logistico usa get_inventory_valuation. Conserva los filtros de producto, marca y stock pedidos; para todo el inventario usa query=null, brand=null y stock_filter=all. Usa totals, nunca sumes solo records ni paginas o fuentes entre si. by_currency separa monedas: cost_reference incluye supuestos de moneda y debe llamarse referencial; cost_verified es solo la parte confirmada. Informa faltantes y fechas. Este valor no es caja, costo de ventas ni una compra nueva. Para movimiento usa el intervalo observado en cada SKU, nunca lo presentes como ventas de un mes diferente.",
