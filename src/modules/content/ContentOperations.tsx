...
import { AlertTriangle, BarChart3, CheckCircle2, Download, Eye, FileSpreadsheet, History, Send, X } from "lucide-react";
...
export function ContentPublications({ data }: { data: ContentCenterData }) {
  ...
  const setStatus = (value: string) => { const next = new URLSearchParams(params); next.set("status", value); setParams(next); };
  const publicationId = params.get("publication"), from = params.get("from");
  const [channelId, setChannelId] = useState("");
  const [selected, setSelected] = useState<ContentPublication | null>(null);
  const [busy, setBusy] = useState("");
  const [notice, setNotice] = useState("");
  const [error, setError] = useState("");
  const filtered = data.publications.filter((item) => (!publicationId || item.id === publicationId) && (!status || item.status === status) && (!channelId || item.channel_id === channelId)
    && (!from || (Number.isFinite(Date.parse(from)) && Boolean(item.published_at) && Date.parse(item.published_at!) >= Date.parse(from))));
