...
function PublicationDetail({ publication, data, onClose }: { publication: ContentPublication; data: ContentCenterData; onClose: () => void }) {
  return <section className="panel content-publication-detail"><div className="panel-heading"><div><h2>{productName(data, publication.product_id)}</h2><span>{channelName(data, publication.channel_id)} · {statusLabel(publication.status)}</span></div><button className="ghost-button" type="button" onClick={onClose}>Cerrar</button></div><div className="content-publication-preview">{publication.image_url ? <img src={publication.image_url} alt="Producto" /> : null}<div><p>{publication.body}</p><div className="content-hashtags">{publication.hashtags.map((tag) => <span key={tag}>#{tag}</span>)}</div>{publication.cta ? <strong>{publication.cta}</strong> : null}</div></div><dl className="content-facts"><div><dt>Modelo</dt><dd>{publication.model_name || "No informado"}</dd></div><div><dt>Creado</dt><dd>{formatDateTime(publication.created_at)}</dd></div><div><dt>Programado</dt><dd>{formatDateTime(publication.scheduled_at)}</dd></div><div><dt>Publicado</dt><dd>{formatDateTime(publication.published_at)}</dd></div><div><dt>ID externo</dt><dd>{publication.external_id || "Pendiente"}</dd></div><div><dt>Reintentos</dt><dd>{publication.retry_count}</dd></div></dl>{publication.error_message ? <div className="notice-banner error"><AlertTriangle size={18} /> {publication.error_message}</div> : null}</section>;
}

export function ContentHistory({ data }: { data: ContentCenterData }) {
  const [level, setLevel] = useState("");
  const [query, setQuery] = useState("");
  const rows = data.history.filter((event) => (!level || event.level === level) && (!query || `${event.message} ${event.event_type}`.toLowerCase().includes(query.toLowerCase())));
  return <div className="content-view-stack"><section className="content-calendar-filters"><label><span>Buscar</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Evento o mensaje" /></label><label><span>Nivel</span><select value={level} onChange={(event) => setLevel(event.target.value)}><option value="">Todos</option><option value="info">Información</option><option value="warning">Advertencia</option><option value="error">Error</option></select></label></section><section className="panel content-history-timeline">{rows.map((event) => <article key={event.id} className={event.level}><div>{event.level === "error" ? <AlertTriangle size={18} /> : event.level === "warning" ? <AlertTriangle size={18} /> : <CheckCircle2 size={18} />}</div><div><strong>{event.message}</strong><span>{event.event_type} · {event.actor_type}{event.actor_id ? ` · ${event.actor_id}` : ""}</span>{Object.keys(event.metadata).length ? <details><summary>Detalle técnico</summary><pre>{JSON.stringify(event.metadata, null, 2)}</pre></details> : null}</div><time>{formatDateTime(event.created_at)}</time></article>)}{!rows.length ? <div className="empty-state"><History size={28} /><span>No hay eventos con estos filtros.</span></div> : null}</section></div>;
}
