  ...
  return (
    <div className="foreign-trade-view-stack">
      ...
      <section className="panel foreign-trade-table-panel">
        <div className="table-scroll"><table className="foreign-trade-operations-table"><thead><tr><th>Operación</th><th>Tipo</th><th>Proveedor</th><th>Estado</th><th>Valor</th><th>Tipo de cambio</th><th>Actualización</th><th aria-label="Acciones" /></tr></thead><tbody>
          {filtered.map((operation) => {
            ...
            return <tr key={operation.id}><td data-label="Operación"><strong>{operation.title}</strong><span>{operation.reference}</span></td><td data-label="Tipo">{operationTypeLabel(operation.operation_type)}</td><td data-label="Proveedor">{supplier?.name || "Sin proveedor"}</td><td data-label="Estado"><span className={`foreign-trade-status ${operationStatus?.color || "neutral"}`}>{operationStatus?.name || operation.status}</span></td><td data-label="Valor">{formatUsd(operation.value_usd)}</td><td data-label="Tipo de cambio">{operation.exchange_rate_clp ? `$${formatDecimal(operation.exchange_rate_clp)} CLP` : "Falta configurar"}</td><td data-label="Actualización">{formatDate(operation.updated_at)}</td><td data-label="Acciones"><div className="foreign-trade-row-actions"><button className="icon-button" type="button" title="Abrir ficha" aria-label={`Abrir ${operation.title}`} onClick={() => onOpen(operation.id)}><Eye size={17} /></button><button className="icon-button danger" type="button" title="Eliminar operación y datos relacionados" aria-label={`Eliminar ${operation.title}`} onClick={() => void onDelete(operation)}><Trash2 size={17} /></button></div></td></tr>;
          })}
        </tbody></table></div>
