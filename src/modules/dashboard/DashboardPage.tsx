  ...
  return <main className="overview-page">
    ...
    {financeAccess && <section className="overview-section" aria-label="Rendimiento financiero">
      ...
      <div className="overview-performance">
        ...
        <div className="overview-results"><h3>{periodLabel}</h3><Result to={reportTo} label="Ventas" value={money(result?.sales)} /><Result to={reportTo} label="Costo de ventas" value={money(result?.costs)} /><Result to={reportTo} label="Gastos operacionales" value={money(result?.expenses)} /><Result to={reportTo} label="Margen bruto" value={result?.grossMargin == null ? "Sin base" : `${number(result.grossMargin)}%`} /><Link className="overview-result-total" to={reportTo}><span>Resultado operativo<br /><small>Provisional</small></span><strong>{money(result?.operatingProfit)}</strong><ArrowUpRight size={18} /></Link></div>
      </div>
