...
export function DashboardPage() {
  ...
  const grossMargin = knownAmount(result?.grossMargin);
  const grossProfit = knownAmount(result?.grossProfit);
  const hasMarginInputs = knownAmount(result?.sales) != null && knownAmount(result?.costs) != null;
  const grossMarginLabel = !result || !hasMarginInputs ? "No disponible" : result.sales <= 0 ? "Sin base"
    : grossMargin == null ? "No disponible" : `${number(grossMargin)}%`;
  const grossMarginDetail = result && hasMarginInputs && result.sales > 0 && grossMargin != null
    ? `Provisional · ${grossProfit == null ? "" : `${money(grossProfit)} de utilidad bruta · `}${result.costs === 0 && ((result.salesCostMissingDocuments || 0) > 0 || hasPendingSales) ? "sin costos incorporados" : "con costos registrados"}`
    : result && hasMarginInputs && result.sales <= 0 ? "Sin ventas netas positivas en el período" : undefined;
  ...
  const detailTo = (metric: DashboardMetric) => dashboardDetailLink(metric, from, to);
  ...
  return <main className="overview-page">
    ...
    {financeAccess && <section className="overview-section" aria-label="Tesorería y cartera">
      ...
      <div className="overview-kpis">
        ...
        <Kpi to="/finanzas-contabilidad?view=receivables&scope=outstanding" icon={CircleDollarSign} title="Por cobrar" value={f?.summary.receivables_suppressed ? "En revisión" : money(f?.summary.receivables)} detail={f?.summary.receivables_data_quality === "verified_full_snapshot" ? "Cartera completa verificada con Facto" : "Cartera operativa · revisar respaldo"} tone="green" />
        <Kpi to="/finanzas-contabilidad?view=payables&scope=outstanding" icon={FileCheck2} title="Por pagar" value={money(f?.summary.payables)} detail="Obligaciones registradas" tone="coral" />
        <Kpi to="/finanzas-contabilidad?view=checks&status=portfolio" icon={FileCheck2} title="Cheques en cartera" value={money(f?.summary.checks_portfolio)} detail="No incluidos en dinero disponible" tone="violet" />
      </div>
    ...
    {financeAccess && <section className="overview-section" aria-label="Rendimiento financiero">
      ...
      <div className="overview-performance">
        ...
        <div className="overview-results"><h3>{periodLabel}</h3>
          {hasSalesBridge && <div className="overview-period-sales" aria-label="Ventas emitidas y ajustes contables">
            <Result to={detailTo("sales-issued")} label="Ventas emitidas en el período" value={money(result?.salesIssued)} detail={`${number(result?.salesIssuedDocuments)} documento(s) de venta · sin IVA ni notas de crédito`} />
            <Result to={detailTo("sales-issued-credit")} label="Notas emitidas en el período" value={deduction(result?.salesIssuedCreditNotes)} />
            {hasSalesAdjustments && <>
              <Result to={detailTo("sales-period-net")} label="Venta neta por fecha de emisión" value={money(result?.salesPeriodNet)} />
              <Result to={detailTo("prior-credit")} label="Notas de crédito de otros períodos" value={money(result?.salesPriorCreditAdjustments)} detail="Contabilizadas en este período" />
              <Result to={detailTo("sales-adjustments")} label="Otras regularizaciones de ventas" value={money(result?.salesOtherAdjustments)} detail="Reversas y diferencias de período" />
            </>}
          </div>}
          <Result to={detailTo("sales-result")} label={hasSalesBridge ? "Ventas netas en resultado" : "Ventas netas"} value={money(result?.sales)} detail={hasSalesAdjustments ? "Incluye las regularizaciones indicadas" : undefined} />
          <Result to={detailTo("sales-credit")} label="Notas de crédito de venta" value={deduction(result?.salesCreditNotes)} detail="Ya descontadas de ventas netas" />
          {!!result?.salesCreditPriorInvoiceDocuments && <Result to={detailTo("credit-prior-invoices")} label="De ellas, sobre facturas de períodos anteriores" value={deduction(result?.salesCreditPriorInvoices)} detail={`${number(result.salesCreditPriorInvoiceDocuments)} notas ya incluidas en ventas netas; no se descuentan nuevamente`} />}
          {hasPendingSales && <div className="overview-sales-breakdown"><Result to={detailTo("sales-ledger")} label="Contabilizadas en CRM" value={money(result?.salesLedger)} /><Result to={detailTo("sales-pending")} label={`Asiento pendiente en CRM · ${number(result?.salesPendingDocuments)} documentos`} value={money(result?.salesPending)} /><p>Incluidas en ventas. Su asiento puede existir en Facto sin haberse incorporado al CRM.</p></div>}
          <Result to={detailTo((result?.salesCostMissingDocuments || 0) > 0 ? "cost-missing" : "costs")} label="Costo de ventas" value={money(result?.costs)} detail={(result?.salesCostMissingDocuments || 0) > 0 ? `${number(result?.salesCostMissingDocuments)} documento(s) con costo pendiente en CRM` : undefined} />
          {!!result?.costCreditNotes && <Result to={detailTo("costs")} label="Ajustes de costo por notas de crédito" value={money(-result.costCreditNotes)} detail="Ya incluidos en el costo de ventas" />}
          {!!result?.creditNoteCostPending && <Result to={detailTo("credit-cost-review")} label="Costos de notas de crédito por verificar" value={`${number(result.creditNoteCostPending)} notas`} detail="Revisar factura y reversa en Facto" />}
          <Result to={detailTo("expenses")} label="Gastos operacionales" value={money(result?.expenses)} /><Result to={detailTo("gross-profit")} label="Margen bruto" value={grossMarginLabel} detail={grossMarginDetail} /><Link className="overview-result-total" to={detailTo("operating-profit")}><span>Resultado operativo contable<br /><small>{hasPendingSales ? "Provisional · base mixta" : "Provisional"}{hasSalesAdjustments ? " · incluye regularizaciones" : ""}{result?.creditNoteCostPending ? " · notas de credito por revisar" : ""}</small></span><strong>{money(result?.operatingProfit)}</strong><ArrowUpRight size={18} /></Link></div>
      </div>
      <div className="overview-purchases">
        <div className="overview-purchases-totals"><h3>Compras · {periodLabel}</h3>
          <Result to={detailTo("purchases")} label="Compras netas" value={money(purchases.net)} />
          <Result to={detailTo("domestic")} label="Compras nacionales" value={money(purchases.domestic)} />
          <Result to={detailTo("international")} label="Compras internacionales" value={money(purchases.international)} detail="Facturas extranjeras netas, sin duplicar recepciones" />
          <Result to={detailTo("purchase-credit")} label="Notas de crédito de compra" value={deduction(result?.purchaseCreditNotes)} detail="Ya descontadas de compras netas" />
        ...
        <details className="overview-purchase-documents">
          ...
          {purchaseDocuments?.length ? <ul>{purchaseDocuments.map(document => <li key={`${document.sourceType || document.kind}:${document.id}`}><Link to={exactDocumentLink(document.id, document.issuedOn)}>
          ...
          <Link className="overview-chart-link" to={detailTo("purchases")}>Compras en fuentes financieras <ArrowUpRight size={16} /></Link>
        </details>
      </div>
      {salesAdjustments.length > 0 && <details className="overview-purchase-documents overview-sales-adjustments">
        ...
        <ul>{salesAdjustments.map(document => <li key={document.id}><Link to={exactDocumentLink(document.id, document.issuedOn)}>
      ...
      {recentSales.length > 0 && <div className="overview-recent-sales"><h3>Últimas ventas registradas</h3>{recentSales.map(sale => <Link key={sale.id} to={exactDocumentLink(sale.id, sale.issuedOn)}><span><strong>Documento {sale.folio}</strong><small>Emisión {date(sale.issuedOn)} · {sale.posted ? "Contabilizado en CRM" : "Asiento pendiente en CRM"}</small></span><strong>{money(sale.netClp)} netos</strong><ArrowUpRight size={16} /></Link>)}</div>}
    ...
    <div className="overview-two">
      ...
      <section className="overview-section"><Heading title="Pulso editorial" detail="Centro de contenido" to="/contenido?view=calendar" /><div className="overview-editorial"><Link to={`/contenido?view=publications&status=published&from=${encodeURIComponent(data.publicationsFrom || "")}`}><strong>{number(counts.published)}</strong><span>Publicadas · últimos 7 días</span></Link><Link to="/contenido?view=publications&status=scheduled"><strong>{number(counts.scheduled)}</strong><span>Programadas</span></Link></div><div className="overview-list">{data.publications.map((publication, index) => <Action key={publication.id} to={`/contenido?view=publications&publication=${encodeURIComponent(publication.id)}`} icon={Palette} label={`Próxima publicación ${index + 1}`} value={date(publication.scheduled_at)} />)}</div><Link className="overview-chart-link" to="/contenido?view=generator">Crear publicación <ArrowUpRight size={16} /></Link></section>
    </div>
    {admin && <section className="overview-section"><Heading title="Fuentes conectadas" detail="Última sincronización informada por cada fuente" to="/administracion" /><div className="overview-connections">{data.connections.map(connection => <Link key={connection.provider} to={`/agentes?focus=connection&provider=${encodeURIComponent(connection.provider)}`}><span className={connection.status === "connected" ? "connected" : "disconnected"} /><div><strong>{({ facto: "Facto", tiendanube: "Tiendanube", gmail: "Gmail", meta_social: "Meta Social", meta_whatsapp: "WhatsApp" } as Record<string, string>)[connection.provider] || connection.provider}</strong><small>{date(connection.last_success_at)}</small></div><span>{connection.status === "connected" ? "Conectada" : "Revisar"}</span><ArrowUpRight size={15} /></Link>)}</div></section>}
