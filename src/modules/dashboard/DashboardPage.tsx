...
export function DashboardPage() {
  ...
  return <main className="overview-page">
    ...
    {financeAccess && <section className="overview-section" aria-label="Rendimiento financiero">
      ...
      {recentSales.length > 0 && <div className="overview-recent-sales"><h3>Últimas ventas registradas</h3>{recentSales.map(sale => <Link key={sale.id} to={exactDocumentLink(sale.id, sale.issuedOn)}><span><strong>Documento {sale.folio}</strong><small>Emisión {date(sale.issuedOn)} · {sale.posted ? "Contabilizado" : "Sin asiento de ingreso"}</small></span><strong>{money(sale.netClp)} netos</strong><ArrowUpRight size={16} /></Link>)}</div>}
