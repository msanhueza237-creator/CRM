...
const documentNormalizer = await readFile(new URL("../supabase/functions/accounting-center/facto-document-normalization.ts", import.meta.url), "utf8");
...
assert.match(documentNormalizer, /receiver_legal_name/);
assert.match(documentNormalizer, /issuer_name/);
assert.match(documentNormalizer, /issuer_tax_id_code/);
assert.match(documentNormalizer, /taxes_amount/);
assert.match(documentNormalizer, /Consumidor final/);
...
assert.match(edgeSource, /existingBankDuplicateRows/);
assert.match(edgeSource, /bankSemanticKey/);
assert.match(edgeSource, /postImportedBankFlow/);
assert.match(edgeSource, /amount <= 2000000/);
assert.match(edgeSource, /bank-balances\/confirm/);
assert.match(edgeSource, /bank-payroll-known-employee:/);
assert.match(edgeSource, /employee_tax_id/);
assert.match(edgeSource, /archivo_origen/);
assert.match(edgeSource, /netResult/);
assert.match(edgeSource, /offset \+= pageSize/);
...
assert.match(edgeSource, /Esto no confirma que falte en Facto/);
assert.match(edgeSource, /Los costos pendientes en el CRM no equivalen a costo cero en Facto/);
...
assert.match(parserSource, /compactLabel/);
assert.match(parserSource, /"numero documento", "n operacion", "numero operacion"/);
assert.match(bankTraceabilityMigration, /accounting_bank_balance_snapshots/);
assert.match(bankTraceabilityMigration, /v_duplicate_count <> 39/);
assert.match(bankTraceabilityMigration, /bank\.import_semantic_duplicates_corrected/);
assert.match(bankTraceabilityMigration, /Sisla Muñoz/);
assert.match(bankTraceabilityMigration, /7733234/);
assert.match(bankTraceabilityMigration, /476138/);
...
assert.match(pageSource, /Pérdidas del período/);
assert.match(pageSource, /Disponible bancario verificado/);
